...
class ToolRenderer {
    ...
    handleEvent(event) {
        switch (event.kind) {
            case 'delta':
                if (event.text)
                    this.stream.markdown(event.text);
                break;
            case 'thinking_delta':
                if (event.text) {
    ...
    handleToolResult(event) {
        ...
        if (!event.tool_name || !event.output)
            return;
        // Skip showing output for these tools (handled elsewhere or too noisy)
        if (event.tool_name === 'write_file' || event.tool_name === 'remember')
            return;
    ...
    captureOldContent(toolCallId, filePath, newContent) {
        let absPath = filePath;
        if (!path.isAbsolute(filePath)) {
            const wsRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            if (wsRoot)
                absPath = path.join(wsRoot, filePath);
        }
...
exports.ToolRenderer = ToolRenderer;
