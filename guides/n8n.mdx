...
| Encryption key mismatch on restart | The encryption key is stored in your unified OS keychain vault (`openpawz`). If you cleared the keychain, delete `~/.openpawz/n8n-data` for a fresh start. |

---

## Security notes

- Your n8n API key is **encrypted** in the Pawz vault (AES-256-GCM, key in unified OS keychain) — never stored in plain text
- The **n8n encryption key** is stored in the unified OS keychain vault (`openpawz`), never on the filesystem in plain text. It’s synced to n8n’s config file only to prevent startup mismatch errors.
