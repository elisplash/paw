...
| **400+ integration nodes** | Planned via MCP Bridge to n8n engine | Medium | Future |
...
|---|---|---|---|
| **Better** | 400+ integrations but no agent intelligence | AI agents but cloud-dependent | Agent-first + 400+ integrations via invisible n8n engine. Agents *decide* when to use integrations. |
...
### Phase 1: Kinetic Design Pass on Flows ← START HERE
Apply all 7 motion primitives to the flow engine:
- [ ] Canvas grain background (`--grain-opacity` dot pattern on SVG canvas)
- [ ] Node materialise on creation (`k-materialise` entrance)
- [ ] Node spring hover (`k-spring` scale-up on mouseover)
- [ ] Breathing indicators (`k-breathe` dots on running/connected nodes)
- [ ] Signal wave on edge activation (`k-signal-fire` sweep along active edges)
- [ ] Halftone pulse on executing nodes (`k-halftone-pulse` overlay)
- [ ] Staggered list entrance (`k-stagger > k-materialise` for flow sidebar)
- [ ] Pixel font for status/kind labels (`Share Tech Mono` for node badges)
- [ ] Oscillating borders on connected nodes (`k-oscillate`)
- [ ] Status color semantic alignment (Running=red, Success=sage, Warning=gold, Idle=steel)

### Phase 2: Flow Templates (20–30)
- [ ] Template data format (extends FlowGraph with category/description/tags)
- [ ] 20–30 curated agent-centric templates
- [ ] Template browser panel in flows sidebar
- [ ] One-click instantiate from template
- [ ] Category filter + search

### Phase 3: Step-by-Step Execution (Debug Mode)
- [ ] "Execute Next Node" button in toolbar
- [ ] Node output inspector panel
- [ ] Execution cursor visualization (which node is next)
- [ ] Breakpoint support (click node to set)
- [ ] Data flow visualization (show values on edges)

### Phase 4: Code Node Kind
- [ ] New `FlowNodeKind: 'code'` with inline JS evaluation
- [ ] Sandboxed execution context (no `window`, no `document`, limited `fetch`)
- [ ] Code editor in config panel (monospace, syntax highlighting optional)
- [ ] Input/output data mapping (previous node output → code input → next node input)

### Phase 5: Schedule Trigger Wiring
- [ ] Connect existing task scheduler / cron system to flow trigger nodes
- [ ] UI: cron expression picker in trigger node config panel
- [ ] Auto-start flow on schedule match
- [ ] Trigger history log (last N fires)

### Phase 6: Error Handling Edges
- [ ] Error output port on every node (separate from success output)
- [ ] Retry config per node (count, delay, backoff)
- [ ] Fallback path (error edge → recovery node)
- [ ] Flow-level error handler (catch-all output)
- [ ] Error node kind (logs, alerts, sends notification)
