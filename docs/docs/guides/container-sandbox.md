...
description: Isolate agent shell commands in ephemeral Docker containers for secure execution.
---
...
Agent calls `exec` tool
        ↓
Sandbox enabled?
  ├── NO → Run directly on host
  └── YES → Create ephemeral Docker container
                ↓
