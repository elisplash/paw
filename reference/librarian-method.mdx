...
The Librarian Method discovers tools via in-memory embedding vectors. [Phase 2 of the Agent Execution Architecture](/reference/agent-execution-roadmap) extends this with the **Persistent Tool Registry** — `PersistentToolRegistry` backs the embedding index with SQLite (`tool_embeddings` table) and adds a four-tier search failover:

1. **Vector similarity** — Cosine search on persisted embeddings
...
The Librarian's in-memory `ToolIndex` remains the hot path. The Persistent Tool Registry adds durability across restarts and scales to 100K+ tools with sub-100ms discovery.
