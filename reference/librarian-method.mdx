...
description: "Intent-Stated Tool Discovery via Local Embeddings — a novel approach to solving tool bloat in agentic AI systems."
sidebarTitle: "The Librarian Method"
---

# The Librarian Method

**Intent-Stated Tool Discovery via Local Embeddings**
...
graph TD
    ...
    subgraph Librarian["Librarian (Ollama · nomic-embed-text)"]
...
| **Librarian** | Ollama `nomic-embed-text` running locally | $0 |
...
Round 1: User says "Email John about the quarterly report"
  ┌─ Agent has: 10 core tools (memory, filesystem, identity, request_tools)
  ├─ Agent understands intent: needs email capabilities
  └─ Agent calls: request_tools({"query": "email sending capabilities"})

  ┌─ Librarian embeds "email sending capabilities" → 768-dim vector
  ├─ Cosine similarity search against tool index
  ├─ Top matches: email_send (0.91), email_read (0.87)
  └─ Domain expansion: email_send → also returns email_read (siblings)

Round 2: Tools hot-loaded into this round
  ┌─ Agent now has: 10 core + email_send + email_read = 12 tools
  ├─ Agent calls: email_send({to: "john@...", subject: "Q4 Report", ...})
  └─ Done ✅

Token cost: ~800 tokens for 12 tool definitions
  vs. ~7,500 tokens if all tools were loaded
  = 89% token savings
```
...
| **Local embeddings** | Zero-cost semantic search via Ollama, no API calls |
...
The Librarian Method requires **zero training**. It uses off-the-shelf embedding models (nomic-embed-text) and works with any tool added to the index — including tools that didn't exist when the embedding model was trained.

---

## Token Economics

| Metric | Without Librarian | With Librarian | Savings |
|--------|-------------------|----------------|---------|
| Tool definitions per request | 75–118 | 10–15 | ~87% |
| Token cost (tool defs) | ~7,500 | ~800 | ~89% |
| Context window freed | — | ~25% of 32K | — |
| API cost per request (est.) | $0.015 | $0.002 | ~87% |
| Embedding search cost | — | $0 (local Ollama) | — |
| Search latency | — | ~50ms | — |

Over thousands of requests, the savings are substantial — both in direct API costs and in context window budget available for actual conversation, memory, and reasoning.
...
The Librarian Method is active by default in OpenPawz. No configuration needed — just have Ollama running with `nomic-embed-text` installed:
...
>
> *The agent calls `request_tools("GitHub notifications")`, the Librarian finds `github_notifications` and `github_list_repos`, and the agent proceeds — using only 12 tool definitions instead of 118.*
