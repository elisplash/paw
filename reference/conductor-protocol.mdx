...
description: "AI-Compiled Flow Execution via Collapse, Extract, Parallelize, Converge, and Tesseract — a novel architecture that treats workflow graphs as blueprints of intent and compiles them into optimized execution strategies."
...
graph TD
    ...
    subgraph Conductor["Conductor Compilation"]
        ...
        K2["5. Tesseract — orchestrate cells across dimensions"]
    end
...
```mermaid actions={false}
graph TD
    subgraph Outer["◻ Cell W=0 — Exploration"]
        C["🔍 Agent C\n(research)"] --> D["📊 Agent D\n(classify)"]
        subgraph Inner["◻ Cell W=1 — Refinement"]
            A["🧠 Agent A\n(analyst)"] <-.-> B["🧠 Agent B\n(critic)"]
        end
    end
    D --> H{"⬡ Event Horizon"}
    B --> H
    H --> Next["✦ Merged Output\nW → 2 (Convergence)"]
...
```mermaid actions={false}
graph LR
    subgraph D2["2D — Flat Flow (X × Y)"]
        a1["A"] --> b1["B"] --> c1["C"]
        a1 --> d1["D"] --> c1
    end
    subgraph D3["3D — Mesh Helix (X × Y × Z)"]
        a2["Agent α"] <-.-> b2["Agent β"]
        b2 -.->|"round 2"| a2
        a2 -->|"round 3"| out2["converged"]
    end
    subgraph D4["4D — Tesseract (X × Y × Z × W)"]
        cell1["Cell W=0"] --> hz["⬡ Horizon"]
        cell2["Cell W=1"] --> hz
        hz --> cell3["Cell W=2"]
    end
    D2 -->|"add cycles"| D3
...
```mermaid actions={false}
graph TD
    subgraph CellA["◻ Cell A — W=0 Exploration"]
        R1["🔍 Research 1"] <-.-> S["👁 Supervisor"]
        R2["🔍 Research 2"] <-.-> S
        R3["🔍 Research 3"] <-.-> S
        S -.->|"Z=0..3\niterate"| R1
    end
    subgraph CellB["◻ Cell B — W=1 Analysis"]
        An1["🧠 Analyst 1"] <-.->|"debate\nZ=0..2"| An2["🧠 Analyst 2"]
    end
    S --> EH{"⬡ Event Horizon\nmerge: synthesize"}
    An2 --> EH
    EH -->|"W → 2"| Conv["✦ Convergence Cell\nAll agents unified"]
...
sequenceDiagram
    ...
    Conductor->>CellA: Promise.all — launch
    Conductor->>CellB: Promise.all — launch
    ...
    Note over Horizon: Merge policy: synthesize<br/>Phase transition: W → 2
...
Tesseract cells operate in different behavioral modes — a research cell and an analysis cell need different long-term memory context. The Conductor provides **per-cell memory isolation** so parallel cells never race on shared state:

1. **Query building** — For each cell, a focused memory query is built from the cell's agent prompts (up to 3 agents, 200 chars each, capped at 300 chars total)
2. **Parallel resolution** — All cell queries are resolved concurrently via `deps.searchMemory()`, which calls `pawEngine.memorySearch()` with score ≥ 0.3 filtering
3. **Immutable context map** — Resolved memory strings are stored in a `resolvedCellMemory` map, computed once before execution begins
4. **Per-cell deps wrapper** — Each cell receives its own `ConductorDeps` wrapper that binds its memory override to `executeNode` and `executeAgentStep`, threading it through the call chain without mutating `runState.memoryContext`

This eliminates the `Promise.all` race condition that would occur if parallel cells mutated a shared field. Each cell's agent nodes receive their own resolved memory context in the `[Relevant Memory]` section of their prompts.

### Execution hardening

Each tesseract cell is protected by multiple safety mechanisms:

| Protection | Detail |
|-----------|--------|
| **Per-cell timeout** | Each cell races against `DEFAULT_CELL_TIMEOUT_MS` (5 min, overridable via `cellTimeoutMs` config). On timeout, the cell produces a `[Cell error: ...]` output instead of blocking the horizon. |
| **Abort propagation** | `if (deps.isAborted()) return;` at the top of each cell task prevents work after flow abort. |
| **Topology-based output** | `findCellSinkNode(cell, graph)` walks edges to find the node with no outgoing edges within the cell — reliable regardless of node insertion order. |
| **Graceful degradation** | Cell failures are caught and recorded as error outputs; the horizon proceeds with partial data rather than crashing the flow. |
| **Circular import prevention** | `conductor-tesseract.ts` imports only `type ExecutionStrategy` from `conductor-atoms.ts` (type-only, erased at runtime). The compiler is injected as a `compileSubgraph?` callback. |
| **BFS caching** | `computeTopoDepth()` runs once per compile and is threaded through all functions via optional `topoDepths?` params — no redundant graph traversals. |
...
| Tesseract | Any event-horizon nodes with cellId assignments |

A 3-node linear flow (trigger → agent → output) skips the Conductor and runs sequentially. A 6-node flow with branches, tool calls, and an agent chain activates the Conductor. A flow with event-horizon nodes and cellId assignments activates the Tesseract primitive regardless of node count. The crossover point for standard flows is approximately 4 nodes — below that, compilation overhead exceeds the savings.
...
5. **Execution** — The strategy runs with all five primitives applied
  ...
  for (const phase of strategy.phases) {
    ...
    await Promise.all(phase.units.map(unit => {
      switch (unit.type) {
        ...
        case 'tesseract':
          return executeTesseractUnit(unit);    // Parallel cells + horizons
      }
    }));
  }
}
...
3. Add an **event-horizon** node from the toolbar (⬡ button)
4. Connect both groups into the event horizon
5. Add an output node after the horizon
6. Run — watch cells execute concurrently, then converge at the horizon with a phase transition
...
## Relationship to the Agent Execution Architecture

The Conductor Protocol operates at the **flow level** — compiling multi-node workflow graphs. The [Agent Execution Architecture](/reference/agent-execution-roadmap) extends parallel execution to the **conversation level** via Action DAG Planning (Phase 0). When a user's request involves multiple independent steps, the agent emits a DAG plan in a single inference call. The engine's DAG executor runs independent branches concurrently — the same principle as the Conductor's Parallelize primitive, applied to individual tool calls within a conversation turn.

Together, the Conductor optimizes compiled flows while Action DAG Planning optimizes ad-hoc agent reasoning — parallel execution at every layer.
