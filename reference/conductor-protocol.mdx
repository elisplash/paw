...
|----------|---------|----------|
| [**The Librarian Method**](/reference/librarian-method) | *Which* tool to use among 25,000+? | Intent-driven discovery via semantic embeddings |
...
2. Agent nodes that need tools use the **Librarian** to discover which tools are relevant from 25,000+ options
...
When n8n nodes are exposed via MCP (Model Context Protocol), they become available as tool, HTTP, and MCP-tool nodes inside OpenPawz flows. But they're no longer confined to sequential, human-designed automations. The Conductor can:

- **Collapse** three sequential n8n-backed agent steps into a single LLM call
- **Extract** n8n tool invocations and run them directly via Rust — no LLM needed
- **Parallelize** independent n8n operations across branches automatically
- **Converge** n8n-connected agents in iterative feedback loops that n8n itself cannot express
...
The Airtable and Slack operations are n8n nodes. They execute via Extract — direct MCP calls, zero LLM cost. The agent steps that need AI intelligence get Collapsed where possible. Independent branches Parallelize automatically.

The 25,000 n8n nodes didn't change. What changed is the engine that orchestrates them.
