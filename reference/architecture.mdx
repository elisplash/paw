...
| `slash-commands` | Chat slash command definitions || `action-dag` | Action DAG plan visualization (Phase 0) |
...
| Module | Purpose |
|--------|---------|
| `agent_loop/` | Agent conversation loop with yield signal, request queue, and plan interception |
| `chat.rs` | System prompt builder, tool assembly, loop detection |
| `state.rs` | Engine state (YieldSignal, RequestQueue, active runs) |
| `providers/` | AI provider factory and routing (with constrained decoding) |
...
| `plan/` | Phase 0: Action DAG planning — DAG validation, cycle detection, parallel executor |
| `constrained/` | Phase 1: Constrained decoding — provider-specific schema enforcement |
| `tool_registry/` | Phase 2: Persistent tool registry — SQLite-backed embeddings, 4-tier failover |
| `binary_ipc/` | Phase 3: Binary IPC — MessagePack EventBatcher + ResultAccumulator |
| `speculative/` | Phase 4: Speculative execution — transition prediction, connection warming |
...
|---------------|-----------|
| `OpenAiProvider` | OpenAI, Ollama, OpenRouter, DeepSeek, Grok, Mistral, Moonshot, Custom. Constrained: `strict: true` on functions |
| `AnthropicProvider` | Anthropic. Constrained: `tool_choice` enforcement |
| `GoogleProvider` | Google Gemini. Constrained: `tool_config` with `function_calling_config` |

`ProviderKind` implements `Copy + Eq` — each provider tracks its kind for phase-specific behavior (e.g., selecting the correct constrained decoding mode).
...
| `error` | `message` | Error during agent execution || `plan_start` | `plan_id`, `node_count` | Action DAG plan execution started (Phase 0) |
| `plan_node_start` | `plan_id`, `node_id`, `tool` | Individual DAG node began execution |
| `plan_complete` | `plan_id`, `results` | Action DAG plan execution finished with all node results |
### Event flow
