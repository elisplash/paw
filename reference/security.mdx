---
title: "Security"
description: "OpenPawz is designed with defense-in-depth: 12 security layers protect against prompt injection, data exfiltration, MITM attacks, memory forensics, agent fixation, and unauthorized actions."
---

# Security

OpenPawz is designed with defense-in-depth: **12 security layers** protect against prompt injection, data exfiltration, MITM attacks, memory forensics, agent fixation, and unauthorized actions.
...
|----------|--------|
| Algorithm | AES-256-GCM (authenticated encryption) |
| Key derivation | HKDF-SHA256 per-agent from OS keychain master key (`paw-memory-vault`) |
