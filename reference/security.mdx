...
- Legacy XOR-encrypted values are auto-migrated to AES-256-GCM on first read
...
Every entry in the security audit log is signed with **HMAC-SHA256** using a dedicated signing key stored in the OS keychain (`paw-audit-chain`), separate from all encryption keys.

| Property | Detail |
|----------|--------|
| Algorithm | HMAC-SHA256 |
| Key storage | OS keychain (`paw-audit-chain`), generated with `OsRng` |
