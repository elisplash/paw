...
The Foreman Protocol reduces cost per tool call. The [Agent Execution Architecture](/reference/agent-execution-roadmap) reduces the *number* of calls:

- **Phase 0 (Action DAG)** — Multiple Foreman-executed tools run in parallel via DAG planning, cutting wall-clock time
- **Phase 1 (Constrained Decoding)** — The Architect's tool-call JSON is guaranteed valid, eliminating retry loops that waste Foreman cycles
- **Phase 4 (Speculative Execution)** — Connections to MCP endpoints are pre-warmed before the Foreman needs them, shaving 200–800ms per tool call

The Foreman handles *how* to execute cheaply; the execution pipeline handles *when* and *how many times*.
