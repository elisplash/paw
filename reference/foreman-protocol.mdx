...
**Bidirectional Access to 25,000+ Services via Local Models and Self-Describing MCP**
...
- **Foreman** (local Ollama model): Interfaces with external services. Handles *how* it happens — both reading and writing. Runs locally at zero API cost.
...
With the Foreman Protocol, the cloud model only handles intent and planning. All service interaction — reading and writing — runs locally. The Architect pays only for the tokens it actually needs frontier intelligence for — understanding the user and deciding what to do. The mechanical work of interfacing with external services shifts to a free local model.

The savings scale with usage. The more your agents interact with connected services, the more you save — because every read and every write that would have burned cloud tokens now runs locally at zero cost.

### vs. Automation Platforms

| Platform | AI-Driven? | Tool Execution Cost | Integrations | Bidirectional? |
|----------|------------|---------------------|--------------|----------------|
| **OpenPawz (Foreman)** | Yes — natural language | Free (local model) | 25,000+ | Yes — read + write, any time |
