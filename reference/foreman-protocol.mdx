...
- **Foreman** (local Ollama model): Executes tool calls. Handles *how* it happens. Runs locally at zero API cost.
...
1. **Architect** (your cloud model) understands the intent and decides to call `mcp_slack_send_message`
2. **Engine** intercepts the `mcp_*` tool call before execution
3. **Foreman** (local Ollama, free) receives the task and the MCP tool schemas
4. **MCP server** (n8n) tells the Foreman what tools are available and exactly how to call them — self-describing
...
With the Foreman Protocol, the cloud model only handles intent and planning. All tool execution runs locally. The Architect pays only for the tokens it actually needs frontier intelligence for — understanding the user and deciding what to do. The mechanical work of formatting and executing tool calls shifts to a free local model.

The savings scale with usage. The more tool calls your agents make, the more you save — because every tool execution that would have burned cloud tokens now runs locally at zero cost.

### vs. Automation Platforms

| Platform | AI-Driven? | Tool Execution Cost | Integrations |
|----------|------------|---------------------|--------------|
| **OpenPawz (Foreman)** | Yes — natural language | Free (local model) | 25,000+ |
| **Zapier** | Partial | Per-task pricing | 7,000 |
| **Make** | No | Per-operation pricing | 2,000 |
| **n8n Standalone** | No — manual workflows | Free (self-hosted) | 400+ built-in |

OpenPawz is the only platform where you can say *"Send a message to #general on Slack every morning with a summary of my GitHub notifications"* and have it work — with natural language, AI-driven execution, and free local tool execution across 25,000+ integrations.
...
The Foreman runs a constrained agent loop — up to 8 rounds of tool calls. This handles multi-step tool execution (e.g., list channels → find channel ID → send message) without risking infinite loops.
...
The Foreman runs any Ollama-compatible model locally. The default `qwen2.5-coder:7b` requires approximately 5 GB of disk and runs comfortably on machines with 8+ GB of RAM (CPU inference) or 5+ GB of VRAM (GPU inference).

On Apple Silicon (M1+), inference is fast enough that tool execution feels instant. On CPU-only systems, it's still typically faster than the round-trip latency of sending the same work to a cloud API and waiting for a response.
...
>
> *Architect identifies the task → Librarian finds n8n QR code node → Foreman executes via MCP → QR code returned — tool execution ran entirely on your machine.*
...
| **Question answered** | *Which* tool to use? | *How* to execute it without cloud costs? |
| **Local model used** | Embedding model (e.g. `nomic-embed-text`) | Code model (e.g. `qwen2.5-coder:7b`) |
