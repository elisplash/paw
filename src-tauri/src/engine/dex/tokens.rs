...
pub(crate) fn resolve_token(symbol_or_address: &str) -> Result<(String, u8), String> {
...
pub(crate) fn resolve_for_swap(symbol_or_address: &str) -> Result<(String, u8, bool), String> {
    let input = symbol_or_address.trim().to_uppercase();
    if input == "ETH" {
