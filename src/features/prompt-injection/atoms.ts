...
const INJECTION_PATTERNS: InjectionPattern[] = [
  ...
  { pattern: /\bignore\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions?|prompts?|rules?|directives?|context)\b/i,
    severity: 'critical', category: 'override',
    description: 'Attempts to override system prompt' },
  { pattern: /\bdisregard\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions?|prompts?|rules?)\b/i,
    severity: 'critical', category: 'override',
    description: 'Disregard previous instructions' },
  { pattern: /\bforget\s+(all\s+)?(previous|prior|your|earlier)\s+(instructions?|prompts?|rules?|training|context)\b/i,
    severity: 'critical', category: 'override',
  ...
  // ── HIGH: Prompt leaking ──
  { pattern: /\b(show|reveal|tell|display|print|output|repeat|echo)\s+(me\s+)?(your|the)\s+(system\s*)?(prompt|instructions?|rules?|configuration|config|context)\b/i,
    severity: 'high', category: 'leaking',
    description: 'Attempts to extract system prompt' },
  { pattern: /\bwhat\s+(are|is)\s+your\s+(system\s*)?(prompt|instructions?|rules?|directives?|guidelines?)\b/i,
    severity: 'high', category: 'leaking',
  ...
  { pattern: /\b(run|execute|call|invoke)\s+(the\s+)?(tool|function|command)\s*[:`'"]/i,
    severity: 'high', category: 'tool_injection',
  ...
  { pattern: /\b(pretend|imagine|suppose|assume)\s+(that\s+)?(you\s+)?(are|have|can|don't|do not)\b/i,
    severity: 'medium', category: 'social',
    description: 'Social engineering — pretend/imagine scenarios' },
  { pattern: /\b(in\s+)?a\s+(hypothetical|fictional|imaginary|roleplay)\s+(scenario|situation|world|context)\b/i,
    severity: 'medium', category: 'social',
  ...
  { pattern: /\bwithout\s+(any\s+)?(restrictions?|limitations?|safet(y|ies)|guardrails?|filters?|censorship)\b/i,
    severity: 'medium', category: 'social',
    description: 'Requesting removal of safety restrictions' },

  // ── MEDIUM: Hidden text / markup abuse ──
  { pattern: /\[INST\]/i,
    severity: 'medium', category: 'markup',
  ...
  { pattern: /\b(bypass|circumvent|evade|override|disable)\s+(the\s+)?(safety|security|content|moderation|filter)/i,
    severity: 'low', category: 'bypass',
